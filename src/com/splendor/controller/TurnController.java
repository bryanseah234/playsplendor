...
public class TurnController {
    ...
    private void executeTakeThreeDifferent(final Move move, final Player player) throws SplendorException {
        ...
        gameView.displayMessage("Took 3 different gems: " + selectedGems);
    }
    ...
    private void executeTakeTwoSame(final Move move, final Player player) throws SplendorException {
        ...
        gameView.displayMessage("Took 2 " + gem + " gems");
    }
    ...
    private void executeReserveCard(final Move move, final Player player) throws SplendorException {
        ...
        if (move.isReservedCard()) {
            // Remove from reserved cards (shouldn't happen in normal gameplay)
            player.removeReservedCard(cardToReserve);
        } else {
        ...
        if (goldAvailable > 0) {
            ...
            gameView.displayMessage("Reserved card and took 1 gold token");
        } else {
            gameView.displayMessage("Reserved card (no gold tokens available)");
        }
    }
    ...
    private void executeBuyCard(final Move move, final Player player) throws SplendorException {
        ...
        gameView.displayMessage("Purchased card: " + cardToBuy);
    }
    ...
    private void executeDiscardTokens(final Move move, final Player player) throws SplendorException {
        ...
        gameView.displayMessage("Discarded tokens: " + tokensToDiscard);
    }
    ...
    private Card findCardToReserve(final Move move, final Board board) {
        // This would need more sophisticated card selection logic
        // For now, return the first available card of the appropriate tier
        final List<Card> availableCards = board.getAvailableCards(1); // Default to tier 1
