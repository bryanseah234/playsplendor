...
powershell -NoProfile -Command " $matches = Get-ChildItem -Recurse -Filter *.md | Select-String -Pattern '```mermaid'; if ($matches) { Write-Host 'Inline mermaid blocks detected. Move source to diagrams/src and reference PNG paths.'; $matches | ForEach-Object { Write-Host (' - ' + $_.Path + ':' + $_.LineNumber) }; exit 3 } "
if %ERRORLEVEL% NEQ 0 exit /b %ERRORLEVEL%
